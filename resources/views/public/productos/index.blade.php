...
@section('title', __('site.products.title').' · '.__('site.brand'))

@section('content')
  <h1 class="mb-4" style="font-weight:800">@lang('site.products.title')</h1>
  @if($productos->count())
    <div class="row">
      @foreach($productos as $p)
        ...
        <div class="col-sm-6 col-md-4 mb-3">
          <div class="glass h-100">
            <div class="position-relative" style="aspect-ratio:4/3; background:#0f0f0f; border-top-left-radius:16px; border-top-right-radius:16px; overflow:hidden">
                ...
                <div class="carousel-inner h-100">
                  ...
                  @forelse($p->imagenes as $k => $img)
                    ...
                    <div class="carousel-item {{ $k === 0 ? 'active' : '' }} h-100">
                      <img src="{{ $url }}" alt="{{ $p->nombre }}" class="d-block w-100" style="height:100%;object-fit:cover">
                    </div>
                  @empty
                ...
                @if($countImgs > 1)
                  ...
                  <ol class="carousel-indicators" style="bottom:6px;">
                    @for($i=0; $i<$countImgs; $i++)
                      <li data-target="#prodCarousel-{{ $p->id }}" data-slide-to="{{ $i }}" class="{{ $i===0 ? 'active' : '' }}"></li>
                    @endfor
                  </ol>
                @endif
              </div>
            ...
            <div class="p-3 position-relative">
              ...
              <div class="font-weight-bold" style="color:green;">{{ number_format($p->precio,2,',','.') }} €</div>
              @if($stock <= 0)
                <div class="mt-1 small text-danger" style="font-weight:600;">@lang('site.products.out_of_stock')</div>
              @elseif($lowStock)
                <div class="mt-1 small text-warning" style="font-weight:600;">@lang('site.products.low_stock', ['count' => $stock])</div>
              @endif
  ...
  @else
    <p class="text-muted">@lang('site.products.no_results')</p>
  @endif
@endsection
